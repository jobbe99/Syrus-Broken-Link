...
class blcCurlHttp extends blcHttpCheckerBase {
	...
	function check($url, $use_get = false){
        ...
        if ( $result['http_code'] == 0 ){
        	...
        	switch( $error_code ) {
        		case 6: //CURLE_COULDNT_RESOLVE_HOST
		        	...
		        	$result['status_text'] = __('Server Not Found', 'syrus-link-checker');
	        	...
	        	case 7: //CURLE_COULDNT_CONNECT
	        		...
	        		if ( $result['request_duration'] >= 0.9*$conf->options['timeout'] ){
	        			...
	        			$result['status_code'] = BLC_LINK_STATUS_WARNING;
	        			$result['status_text'] = __('Connection Failed', 'syrus-link-checker');
        		...
        		default:
	        		$result['status_code'] = BLC_LINK_STATUS_WARNING;
	        		$result['status_text'] = __('Unknown Error', 'syrus-link-checker');
        	}
        ...
        $log .= '=== ';
        if ( $result['http_code'] ){
			$log .= sprintf( __('HTTP code : %d', 'syrus-link-checker'), $result['http_code']);
		} else {
			$log .= __('(No response)', 'syrus-link-checker');
		}
		$log .= " ===\n\n";
        $log .= $this->last_headers;

        if ( !empty($result['broken']) && !empty($result['timeout']) ) {
			$log .= "\n(" . __("Most likely the connection timed out or the domain doesn't exist.", 'syrus-link-checker') . ')';
		}
...
class blcSnoopyHttp extends blcHttpCheckerBase {
	...
	function check($url){
        ...
        if ($snoopy->error)
            $log .= $snoopy->error."\n";
        if ($snoopy->timed_out) {
            $log .= __("Request timed out.", 'syrus-link-checker') . "\n";
            $result['timeout'] = true;
        }
		...
		$log .= "<em>(" . __('Using Snoopy', 'syrus-link-checker') . ")</em>";
		$result['log'] = $log;
