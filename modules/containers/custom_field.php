...
class blcPostMeta extends blcContainer {
	...
	function update_field($field, $new_value, $old_value = ''){
		...
		if ( $rez ){
			...
			return new WP_Error(
				...
				sprintf(
					__("Failed to update the meta field '%s' on %s [%d]", 'syrus-link-checker'), 
					$field, 
					$this->meta_type, 
					$this->container_id
				)
	...
	function unlink($field_name, $parser, $url, $raw_url =''){
		...
		if ( $rez ){
			...
			return new WP_Error(
				...
				sprintf(
					__("Failed to delete the meta field '%s' on %s [%d]", 'syrus-link-checker'), 
					$field_name,
					$this->meta_type, 
					$this->container_id
				)
   ...
   */
	...
	function ui_get_action_links($container_field){
		...
		$actions['view'] = '<span class="view"><a href="' . esc_url(get_permalink($this->container_id)) . '" title="' . esc_attr(sprintf(__('View "%s"', 'syrus-link-checker'), get_the_title($this->container_id))) . '" rel="permalink">' . __('View') . '</a>';
