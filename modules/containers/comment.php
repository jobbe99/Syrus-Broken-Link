...
class blcComment extends blcContainer{
   ...
   */
	function update_wrapped_object(){
		...
		if ( wp_update_comment($data) ){
			...
			return new WP_Error(
				'update_failed',
				sprintf(__('Updating comment %d failed', 'syrus-link-checker'), $this->container_id)
			);
		}
	}
		...
		if ( EMPTY_TRASH_DAYS ){
			...
			if ( wp_delete_comment($this->container_id, true) ){
				...
				return new WP_Error(
					'delete_failed',
					sprintf(
						__('Failed to delete comment %d', 'syrus-link-checker'),
						$this->container_id
					)
				);
			}
	...
	function trash_wrapped_object(){
		if ( wp_trash_comment($this->container_id) ){
			...
			return new WP_Error(
				'trash_failed',
				sprintf(
					__('Can\'t move comment %d to the trash', 'syrus-link-checker'),
					$this->container_id
				)
			);
		}
   ...
   */
	...
	function ui_get_action_links($container_field){
		...
		$actions['view'] = '<span class="view"><a href="' . get_comment_link($this->container_id) . '" title="' . esc_attr(__('View comment', 'syrus-link-checker')) . '" rel="permalink">' . __('View') . '</a>';
	...
	function ui_get_source($container_field = '', $context = 'display'){
		...
		$image = sprintf(
			'<img src="%s/syrus-link-checker/images/%s" class="blc-small-image" title="%3$s" alt="%3$s"> ',
			WP_PLUGIN_URL,
			$image,
			__('Comment', 'syrus-link-checker')
		);
