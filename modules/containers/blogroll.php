...
class blcBookmark extends blcContainer{
	...
	function ui_get_source($container_field = '', $context = 'display'){
		...
		$image = sprintf(
			...
			__('Bookmark', 'syrus-link-checker')						
		);
		
		$link_name = sprintf(
			'<a class="row-title" href="%s" title="%s">%s</a>',
			$this->get_edit_url(),
			__('Edit this bookmark', 'syrus-link-checker'),
	...
	function ui_get_action_links($container_field){
		...
		if ( current_user_can('manage_links') ) {
			$actions['edit'] = '<span class="edit"><a href="' . $this->get_edit_url() . '" title="' . esc_attr(__('Edit this bookmark', 'syrus-link-checker')) . '">' . __('Edit') . '</a>';
   ...
   */
	function update_wrapped_object(){
		...
		if ( !empty($rez) ){
			...
			return new WP_Error(
				'update_failed',
				sprintf(__('Updating bookmark %d failed', 'syrus-link-checker'), $this->container_id)
			);
		}
	}
   ...
   */
	function delete_wrapped_object(){
		if ( wp_delete_link($this->container_id) ){
			...
			$msg = sprintf(
				__('Failed to delete blogroll link "%s" (%d)', 'syrus-link-checker'),
				$link_name,
				$this->container_id
			);
