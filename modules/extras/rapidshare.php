...
class blcRapidShareChecker extends blcChecker {
	...
	function check($url){
		$result = array(
			...
			'timeout' => false,
			'broken' => false,
			'log' => sprintf("<em>(%s)</em>\n\n", __('Using RapidShare API', 'syrus-link-checker')),
			'result_hash' => '',
			'status_code' => '',
			'status_text' => '',
		);
				...
				if ( isset($data[4]) ){
					...
					switch( $file_status ){
						case 0:
							...
							$result['status_text'] = __('Not Found', 'syrus-link-checker');
							break;
							
						case 1:
							$file_status_text = 'File OK (Anonymous downloading)';
							$result['status_code'] = BLC_LINK_STATUS_OK;
							$result['status_text'] = _x('OK', 'link status', 'syrus-link-checker');
							break;
						...
						case 3:
							...
							$result['status_text'] = __('RS Server Down', 'syrus-link-checker');
							break;
						...
						case 4:
							...
							$result['broken'] = true;
							$result['status_code'] = BLC_LINK_STATUS_ERROR;
							$result['status_text'] = __('File Blocked', 'syrus-link-checker');
							break;
						...
						case 5:
							...
							$result['broken'] = true;
							$result['status_code'] = BLC_LINK_STATUS_WARNING;
							$result['status_text'] = __('File Locked', 'syrus-link-checker');
							break;
					...
					$result['log'] .= sprintf(
						__('RapidShare : %s', 'syrus-link-checker'),
						$file_status_text
					);
												
				} else {
					$result['log'] .= sprintf(
						__('RapidShare API error: %s', 'syrus-link-checker'),
						$response['body']
					);
				}			
