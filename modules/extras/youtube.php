...
class blcYouTubeChecker extends blcChecker {
	...
	protected function check_video($response, $result) {
		switch($result['http_code']){
			case 404 : //Not found
				$result['log'] .= __('Video Not Found', 'syrus-link-checker');
				$result['broken'] = true;
				$result['http_code'] = 0;
				$result['status_text'] = __('Video Not Found', 'syrus-link-checker');
			...
			case 403 : //Forbidden. Usually means that the video has been removed. Body contains details.
				...
				$result['broken'] = true;
				$result['http_code'] = 0;
				$result['status_text'] = __('Video Removed', 'syrus-link-checker');
				$result['status_code'] = BLC_LINK_STATUS_ERROR;
				break;
			...
			case 400 : //Bad request. Usually means that the video ID is incorrect. Body contains details.
				...
				$result['broken'] = true;
				$result['http_code'] = 0;
				$result['status_text'] = __('Invalid Video ID', 'syrus-link-checker');
			...
			case 200 : //Video exists, but may be restricted. Check for <yt:state> tags.
				...
				if ( empty($state) ){
					...
					$result['log'] .= __("Video OK", 'syrus-link-checker');
					$result['status_text'] = __('OK', 'link status', 'syrus-link-checker');
					...
					$result['log'] .= sprintf(
						__('Video status : %s%s', 'syrus-link-checker'),
						$state_name,
					...
					if ( $this->is_state_ok($state_name, $state_reason) ) {
						...
						$result['broken'] = true;
						$result['status_text'] = __('Video Restricted', 'syrus-link-checker');
						$result['status_code'] = BLC_LINK_STATUS_WARNING;
						$result['http_code'] = 0;
					}
				}
	...
	protected function check_playlist($response, $result) {
		...
		switch($result['http_code']){
			case 404 : //Not found
				$result['log'] .= __('Playlist Not Found', 'syrus-link-checker');
				$result['broken'] = true;
				$result['http_code'] = 0;
				$result['status_text'] = __('Playlist Not Found', 'syrus-link-checker');
				$result['status_code'] = BLC_LINK_STATUS_ERROR;
				break;
			...
			case 403 : //Forbidden. We're unlikely to see this code for playlists, but lets allow it.
				$result['log'] .= $response['body'];
				$result['broken'] = true;
				$result['status_text'] = __('Playlist Restricted', 'syrus-link-checker');
				$result['status_code'] = BLC_LINK_STATUS_ERROR;
				break;
			...
			case 400 : //Bad request. Probably indicates a client error (invalid API request). Body contains details.
				$result['log'] .= $response['body'];
				$result['broken'] = true;
				$result['status_text'] = __('Invalid Playlist', 'syrus-link-checker');
				$result['status_code'] = BLC_LINK_STATUS_WARNING;
				break;
			...
			case 200 :
				...
				if ( empty($video_states) ){
					...
					if ( !empty($entries) ) {
						//All is well.
						$result['log'] .= __("Playlist OK", 'syrus-link-checker');
						$result['status_text'] = __('OK', 'link status', 'syrus-link-checker');
						$result['status_code'] = BLC_LINK_STATUS_OK;
						$result['http_code'] = 0;
					} else {
						...
						$result['log'] .= __("This playlist has no entries or all entries have been deleted.", 'syrus-link-checker');
						$result['status_text'] = __('Empty Playlist', 'link status', 'syrus-link-checker');
						$result['status_code'] = BLC_LINK_STATUS_WARNING;
						$result['http_code'] = 0;
						$result['broken'] = true;
					}
