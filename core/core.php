...
class wsBrokenLinkChecker {
   ...
   */
    ...
    function dashboard_widget(){
        ?>
        <p id='wsblc_activity_box'><?php _e('Loading...', 'syrus-link-checker');  ?></p>
        <script type='text/javascript'>
        	jQuery( function($){
					...
					$.getJSON(
						...
						function (data){
							if ( data && ( typeof(data.text) != 'undefined' ) ) {
								...
								$('#wsblc_activity_box').html('<?php _e('[ Network error ]', 'syrus-link-checker'); ?>');
							}
    ...
    function dashboard_widget_control(
		...
		<p><label for="blc-autoexpand">
			...
			<?php _e('Automatically expand the widget if broken links have been detected', 'syrus-link-checker'); ?>
		</label></p>
		<?php
    }
   ...
   */
    function initiate_recheck(){
    	...
    	$wpdb->query("TRUNCATE {$wpdb->prefix}syrus_link_checker_instances");
   ...
   */
    ...
    function admin_menu(){
        ...
        $options_page_hook = add_options_page(
			__('Link Checker Settings', 'syrus-link-checker'),
			__('Link Checker', 'syrus-link-checker'),
			'manage_options',
            'link-checker-settings',array($this, 'options_page')
		);

		$menu_title = __('Broken Links', 'syrus-link-checker');
        ...
        $links_page_hook = add_management_page(
			__('View Broken Links', 'syrus-link-checker'),
			$menu_title,
			'edit_others_posts',
        ...
        add_screen_meta_link(
        	'blc-feedback-widget',
        	__('Feedback', 'syrus-link-checker'),
        	'http://whiteshadow.uservoice.com/forums/58400-syrus-link-checker',
		...
		add_screen_meta_link(
        	'blc-links-page-link',
			__('Go to Broken Links', 'syrus-link-checker'),
   ...
   */
    ...
    function options_page(){
		...
		$available_link_actions = array(
			'edit'               => __('Edit URL' , 'syrus-link-checker'),
			'delete'             => __('Unlink', 'syrus-link-checker'),
			'blc-discard-action' => __('Not broken', 'syrus-link-checker'),
			'blc-dismiss-action' => __('Dismiss', 'syrus-link-checker'),
			'blc-recheck-action' => __('Recheck', 'syrus-link-checker'),
			'blc-deredirect-action' => _x('Fix redirect', 'link action; replace one redirect with a direct link', 'syrus-link-checker')
		);
        ...
        if(isset($_POST['submit'])) {
			...
			if ( $this->conf->options['logging_enabled'] ) {
				if ( $this->conf->options['custom_log_file_enabled'] ) {
					...
					//Default log file is /wp-content/uploads/syrus-link-checker/blc-log.txt
        ...
        if ( !empty($_GET['settings-updated']) ){
        	echo '<div id="message" class="updated fade"><p><strong>',__('Settings saved.', 'syrus-link-checker'), '</strong></p></div>';

        }

        //Show a message when recheck is started, too.
        if ( !empty($_GET['recheck-initiated']) ){
        	echo '<div id="message" class="updated fade"><p><strong>',
			     	__('Complete site recheck started.', 'syrus-link-checker'), // -- Yoda
			     '</strong></p></div>';
        }
		...
		$section_names = array(
			'general' =>  __('General', 'syrus-link-checker'),
			'where' =>    __('Look For Links In', 'syrus-link-checker'),
			'which' =>    __('Which Links To Check', 'syrus-link-checker'),
			'how' =>      __('Protocols & APIs', 'syrus-link-checker'),
			'advanced' => __('Advanced', 'syrus-link-checker'),
		);
		?>
		...
		<h2><?php _e('Syrus Link Checker Options', 'syrus-link-checker'); ?></h2>
        ...
        <tr valign="top">
        <th scope="row">
			<?php _e('Status','syrus-link-checker'); ?>
			<br>
			<a href="javascript:void(0)" id="blc-debug-info-toggle"><?php _e('Show debug info', 'syrus-link-checker'); ?></a>
		</th>
        <td>
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Check each link','syrus-link-checker'); ?></th>
        <td>

		<?php
			printf(
				__('Every %s hours','syrus-link-checker'),
				sprintf(
        ...
        <?php _e('Existing links will be checked this often. New links will usually be checked ASAP.', 'syrus-link-checker'); ?>
        </span>
        ...
        <tr valign="top">
        <th scope="row"><?php _e('E-mail notifications', 'syrus-link-checker'); ?></th>
        <td>
        	...
        	<label for='send_email_notifications'>
            	...
            	<?php _e('Send me e-mail notifications about newly detected broken links', 'syrus-link-checker'); ?>
			</label><br />
			</p>
        	...
        	<label for='send_authors_email_notifications'>
            	...
            	<?php _e('Send authors e-mail notifications about broken links in their posts', 'syrus-link-checker'); ?>
			</label><br />
			</p>
        </td>
        </tr>
		...
		<tr valign="top">
			<th scope="row"><?php echo __('Notification e-mail address', 'syrus-link-checker'); ?></th>
			<td>
				<p>
				<label>
					<input
				...
				<span class="description">
					<?php echo __('Leave empty to use the e-mail address specified in Settings &rarr; General.', 'syrus-link-checker'); ?>
				</span>
				</p>
			</td>
		</tr>
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Link tweaks','syrus-link-checker'); ?></th>
        <td>
        	...
        	<label for='mark_broken_links'>
            	...
            	<?php _e('Apply custom formatting to broken links', 'syrus-link-checker'); ?>
			</label>
			|
			<a id="toggle-broken-link-css-editor" href="#" class="blc-toggle-link"><?php
				_e('Edit CSS', 'syrus-link-checker');
			?></a>
			</p>
			...
			?>>
		        ...
		        <p class="description"><?php
					printf(
						__('Example : Lorem ipsum <a %s>broken link</a>, dolor sit amet.', 'syrus-link-checker'),
						' href="#" class="broken_link" onclick="return false;"'
					);
					echo ' ', __('Click "Save Changes" to update example output.', 'syrus-link-checker');
				?></p>
        	</div>
        	...
        	<label for='mark_removed_links'>
            	...
            	<?php _e('Apply custom formatting to removed links', 'syrus-link-checker'); ?>
			</label>
			|
			...
			?>>
		        ...
		        <p class="description"><?php
			    printf(
			        __('Example : Lorem ipsum <span %s>removed link</span>, dolor sit amet.', 'syrus-link-checker'),
			        ' class="removed_link"'
		        );
				echo ' ', __('Click "Save Changes" to update example output.', 'syrus-link-checker');
				?>
        	...
        	<label for='nofollow_broken_links'>
            	...
            	<?php _e('Stop search engines from following broken links', 'syrus-link-checker'); ?>
			</label>
			</p>
			...
			<tr valign="top">
				<th scope="row"><?php echo _x('Suggestions', 'settings page', 'syrus-link-checker'); ?></th>
				<td>
					<label>
						<input type="checkbox" name="suggestions_enabled" id="suggestions_enabled"
							<?php checked($this->conf->options['suggestions_enabled']); ?>/>
						<?php _e('Suggest alternatives to broken links', 'syrus-link-checker'); ?>
					</label>
				</td>
			</tr>

			<tr valign="top">
				<th scope="row"><?php echo _x('Warnings', 'settings page', 'syrus-link-checker'); ?></th>
				<td id="blc_warning_settings">
					<label>
						...
						<?php _e('Show uncertain or minor problems as "warnings" instead of "broken"', 'syrus-link-checker'); ?>
					</label>
					<p class="description"><?php
						_e('Turning off this option will make the plugin report all problems as broken links.', 'syrus-link-checker');
					?></p>
				</td>
			</tr>
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Look for links in', 'syrus-link-checker'); ?></th>
        <td>
    	<?php
    	...
    	<tr valign="top">
        <th scope="row"><?php _e('Post statuses', 'syrus-link-checker'); ?></th>
        <td>
    	<?php
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Link types', 'syrus-link-checker'); ?></th>
        <td>
        <?php
        if ( !empty($modules['parser']) ){
        	$this->print_module_list($modules['parser'], $this->conf->options);
        } else {
        	echo __('Error : All link parsers missing!', 'syrus-link-checker');
        }
    	?>
    	</td>
		</tr>
    	...
    	<tr valign="top">
        <th scope="row"><?php _e('Exclusion list', 'syrus-link-checker'); ?></th>
        <td><?php _e("Don't check links where the URL contains any of these words (one per line) :", 'syrus-link-checker'); ?><br/>
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Check links using', 'syrus-link-checker'); ?></th>
        <td>
        <?php
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Timeout', 'syrus-link-checker'); ?></th>
        <td>

		<?php

		printf(
			__('%s seconds', 'syrus-link-checker'),
			sprintf(
        ...
        <?php _e('Links that take longer than this to load will be marked as broken.','syrus-link-checker'); ?>
		</span>
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Link monitor', 'syrus-link-checker'); ?></th>
        <td>
			...
			<label for='run_in_dashboard'>
	            	...
	            	<?php _e('Run continuously while the Dashboard is open', 'syrus-link-checker'); ?>
			</label>
			</p>
			...
			<label for='run_via_cron'>
	            	...
	            	<?php _e('Run hourly in the background', 'syrus-link-checker'); ?>
			</label>
			</p>
        ...
        <tr valign="top">
	        <th scope="row"><?php _e('Show the dashboard widget for', 'syrus-link-checker'); ?></th>
	        <td>

		        <?php
				$widget_caps = array(
					_x('Administrator', 'dashboard widget visibility', 'syrus-link-checker') => 'manage_options',
					_x('Editor and above', 'dashboard widget visibility', 'syrus-link-checker') => 'edit_others_posts',
					_x('Nobody (disables the widget)', 'dashboard widget visibility', 'syrus-link-checker') => 'do_not_allow',
				);
		...
		<tr valign="top">
			<th scope="row"><?php echo _x('Show link actions', 'settings page', 'syrus-link-checker'); ?></th>
			<td>
				<?php
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Max. execution time', 'syrus-link-checker'); ?></th>
        <td>
        ...
        _e('The plugin works by periodically launching a background job that parses your posts for links, checks the discovered URLs, and performs other time-consuming tasks. Here you can set for how long, at most, the link monitor may run each time before stopping.', 'syrus-link-checker');
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Server load limit', 'syrus-link-checker'); ?></th>
        <td>
		<?php
		...
		if ( $available ){
			...
			printf(
				__('Current load : %s', 'syrus-link-checker'),
        	...
        	echo '<input type="text" disabled="disabled" value="', esc_attr(__('Not available', 'syrus-link-checker')), '" size="13"/><br>';
        	echo '<span class="description">';
			_e('Load limiting only works on Linux-like systems where <code>/proc/loadavg</code> is present and accessible.', 'syrus-link-checker');
			echo '</span>';
		}
		?>
        </td>
		...
		<tr valign="top">
			<th scope="row"><?php _e('Target resource usage', 'syrus-link-checker'); ?></th>
			<td>
				<?php
		...
		<tr valign="top">
			<th scope="row"><?php _e('Logging', 'syrus-link-checker'); ?></th>
			<td>
				<p>
					<label for='logging_enabled'>
						...
						<?php _e('Enable logging', 'syrus-link-checker'); ?>
					</label>
				</p>
			</td>
		</tr>
		...
		<tr valign="top">
			<th scope="row"><?php _e('Log file location', 'syrus-link-checker'); ?></th>
			<td>
				...
				<label>
					...
					<?php echo _x('Default', 'log file location', 'syrus-link-checker'); ?>
				</label>
				<br>
					<span class="description">
						<code><?php
				...
				<label>
					<input type="radio" name="custom_log_file_enabled" value="1"
						<?php checked($this->conf->options['custom_log_file_enabled']); ?>>
					<?php echo _x('Custom', 'log file location', 'syrus-link-checker'); ?>
				</label>
        ...
        <tr valign="top">
        <th scope="row"><?php _e('Forced recheck', 'syrus-link-checker'); ?></th>
        <td>
        	<input class="button" type="button" name="start-recheck" id="start-recheck"
				  value="<?php _e('Re-check all pages', 'syrus-link-checker'); ?>"  />
  			<input type="hidden" name="recheck" value="" id="recheck" />
			<br />
  			<span class="description"><?php
			  _e('The "Nuclear Option". Click this button to make the plugin empty its link database and recheck the entire site from scratch.', 'syrus-link-checker');
    	...
    	foreach($modules as $module_id => $module_data){
			...
			if ( !empty($extra_settings) ){
				...
				printf(
					...
					$module_id,
					__('Configure', 'syrus-link-checker')
				);
    ...
    function links_page(){
		...
		if ( empty($current_filter['links']) && !empty($wpdb->last_error) ){
			printf( __('Database error : %s', 'syrus-link-checker'), $wpdb->last_error);
		}
        ?>
...
<div class="wrap">
   ...
   */
    function do_create_custom_filter(){
		...
		if ( empty($_POST['name']) ){
			$message = __("You must enter a filter name!", 'syrus-link-checker');
			$msg_class = 'error';
		//Filter parameters (a search query) must also be set
		} elseif ( empty($_POST['params']) ){
			$message = __("Invalid search query.", 'syrus-link-checker');
			$msg_class = 'error';
		} else {
			//Save the new filter
			$blc_link_query = blcLinkQuery::getInstance();
			...
			if ( $filter_id ){
				//Saved
				$message = sprintf( __('Filter "%s" created', 'syrus-link-checker'), $_POST['name']);
				//A little hack to make the filter active immediately
				$_GET['filter_id'] = $filter_id;
			} else {
				//Error
				$message = sprintf( __("Database error : %s", 'syrus-link-checker'), $wpdb->last_error);
				$msg_class = 'error';
			}
		}
   ...
   */
	function do_delete_custom_filter(){
		...
		if ( empty($_POST['filter_id']) ){
			$message = __("Filter ID not specified.", 'syrus-link-checker');
			$msg_class = 'error';
		} else {
			...
			if ( $blc_link_query->delete_custom_filter($_POST['filter_id']) ){
				//Success
				$message = __('Filter deleted', 'syrus-link-checker');
			} else {
				//Either the ID is wrong or there was some other error
				$message = __('Database error : %s', 'syrus-link-checker');
				$msg_class = 'error';
			}
		}
	...
	function do_bulk_deredirect($selected_links){
		...
		if ( count($selected_links) > 0 ) {
			...
			if ( count($links) > 0 ) {
				...
				$message = __('None of the selected links are redirects!', 'syrus-link-checker');
			}
		}
	...
	function do_bulk_delete_sources($selected_links, $force_delete = false){
		...
		if ( count($selected_links) > 0 ) {
			...
			if ( count($messages) > 0 ){
				...
				$message = __("Didn't find anything to delete!", 'syrus-link-checker');
				$msg_class = 'error';
			}
		}
   ...
   */
	...
	function do_bulk_discard($selected_links){
		...
		if ( count($selected_links) > 0 ){
			foreach($selected_links as $link_id){
				...
				$link->log = __("This link was manually marked as working by the user.", 'syrus-link-checker');
				...
				if ( $link->save() ){
					...
					$messages[] = sprintf(
						__("Couldn't modify link %d", 'syrus-link-checker'),
						$link_id
					);
					$msg_class = 'error';
				}
	...
	function screen_options_html(){
		...
		$html = '<h5>' . __('Table columns', 'syrus-link-checker') . '</h5>';
		...
		$html .= sprintf(
			...
			$this->conf->options['table_links_per_page'],
			__('links', 'syrus-link-checker'),
			__('Apply')
		);
		$html .= '</div>';

		$html .= '<h5>' . __('Misc', 'syrus-link-checker') . '</h5>';
		$html .= '<div class="screen-options">';
		/*
		...
		*/
        ...
        $html .= sprintf(
			__('Highlight links broken for at least %s days', 'syrus-link-checker'),
			$input_box
		);
		$html .= '</label>';
		...
		$html .= sprintf(
			...
			__('Color-code status codes', 'syrus-link-checker')
		);
	...
	function ajax_save_screen_options($form){
		if ( !current_user_can('edit_others_posts') ){
			die( json_encode( array(
				'error' => __("You're not allowed to do that!", 'syrus-link-checker')
			 )));
		}
	...
	function get_links_to_check($max_results = 0, $count_only = false){
		...
		$q .= "FROM {$wpdb->prefix}blc_links AS links
		      WHERE
				...
				AND EXISTS (
					SELECT 1 FROM {$wpdb->prefix}syrus_link_checker_instances AS instances
					WHERE
   ...
   */
	function status_text( $status ){
		...
		if( $status['broken_links'] > 0 ){
			$text .= sprintf(
				"<a href='%s' title='" . __('View broken links', 'syrus-link-checker') . "'><strong>".
					_n('Found %d broken link', 'Found %d broken links', $status['broken_links'], 'syrus-link-checker') .
				"</strong></a>",
			  	admin_url('tools.php?page=view-broken-links'),
				$status['broken_links']
			);
		} else {
			$text .= __("No broken links found.", 'syrus-link-checker');
		}
		...
		if( $status['unchecked_links'] > 0) {
			$text .= sprintf(
				_n('%d URL in the work queue', '%d URLs in the work queue', $status['unchecked_links'], 'syrus-link-checker'),
				$status['unchecked_links'] );
		} else {
			$text .= __("No URLs in the work queue.", 'syrus-link-checker');
		}

		$text .= "<br/>";
		if ( $status['known_links'] > 0 ){
			$url_count = sprintf(
				_nx('%d unique URL', '%d unique URLs', $status['known_links'], 'for the "Detected X unique URLs in Y links" message', 'syrus-link-checker'),
				$status['known_links']
			);
			$link_count = sprintf(
				_nx('%d link', '%d links', $status['known_instances'], 'for the "Detected X unique URLs in Y links" message', 'syrus-link-checker'),
				$status['known_instances']
			);

			if ($this->conf->options['need_resynch']){
				$text .= sprintf(
					__('Detected %1$s in %2$s and still searching...', 'syrus-link-checker'),
					$url_count,
					$link_count
				);
			} else {
				$text .= sprintf(
					__('Detected %1$s in %2$s.', 'syrus-link-checker'),
					$url_count,
					$link_count
				);
			}
			...
			if ($this->conf->options['need_resynch']){
				$text .= __('Searching your blog for links...', 'syrus-link-checker');
			} else {
				$text .= __('No links detected.', 'syrus-link-checker');
			}
		}
   ...
   */
	function ajax_current_load(){
		$load = blcUtility::get_server_load();
		if ( empty($load) ){
			die( _x('Unknown', 'current load', 'syrus-link-checker') );
		}
   ...
   */
	function ajax_discard(){
		if (!current_user_can('edit_others_posts') || !check_ajax_referer('blc_discard', false, false)){
			die( __("You're not allowed to do that!", 'syrus-link-checker') );
		}
		...
		if ( isset($_POST['link_id']) ){
			...
			if ( !$link->valid() ){
				printf( __("Oops, I can't find the link %d", 'syrus-link-checker'), intval($_POST['link_id']) );
				die();
			}
			//Make it appear "not broken"
			$link->broken = false;
			...
			if ( $link->save() ){
				...
				die( __("Oops, couldn't modify the link!", 'syrus-link-checker') ) ;
			}
		} else {
			die( __("Error : link_id not specified", 'syrus-link-checker') );
		}
	}
   ...
   */
	function ajax_edit(){
		...
		if ( empty($_POST['link_id']) || empty($_POST['new_url']) || !is_numeric($_POST['link_id']) ) {
			die( json_encode( array(
				'error' => __("Error : link_id or new_url not specified", 'syrus-link-checker')
			)));
		}
		...
		if ( !$link->valid() ){
			die( json_encode( array(
				'error' => sprintf( __("Oops, I can't find the link %d", 'syrus-link-checker'), intval($_POST['link_id']) )
			)));
		}
		...
		if ( !$parsed ){
			die( json_encode( array(
				'error' => __("Oops, the new URL is invalid!", 'syrus-link-checker')
			)));
		}
		...
		if ( $rez === false ){
			die( json_encode( array(
				'error' => __("An unexpected error occurred!", 'syrus-link-checker')
			)));
		} else {
   ...
   */
	function ajax_unlink(){
		...
		if ( isset($_POST['link_id']) ){
			...
			if ( $rez === false ){
				die( json_encode( array(
					'error' => __("An unexpected error occured!", 'syrus-link-checker')
				 )));
			} else {
				$response = array(
					'cnt_okay' => $rez['cnt_okay'],
			...
			die( json_encode( array(
					'error' => __("Error : link_id not specified", 'syrus-link-checker')
				 )));
		}
	}
	...
	public function ajax_deredirect() {
		...
		if ( !$link->valid() ){
			die( json_encode( array(
				'error' => sprintf(__("Oops, I can't find the link %d", 'syrus-link-checker'), $id)
			)));
		}
	...
	function ajax_link_details(){
		...
		if (!current_user_can('edit_others_posts')){
			die( __("You don't have sufficient privileges to access this information!", 'syrus-link-checker') );
		}
		...
		if ( isset($_GET['link_id']) ){
			...
			die( __('Error : link ID not specified', 'syrus-link-checker') );
		}
		...
		if ( !$link->is_new ){
			...
			printf( __('Failed to load link details (%s)', 'syrus-link-checker'), $wpdb->last_error );
			die();
		}
	}
	...
	function hook_wp_dashboard_setup(){
		...
		if ( function_exists( 'wp_add_dashboard_widget' ) && $show_widget ) {
			wp_add_dashboard_widget(
				...
				__('Syrus Link Checker', 'syrus-link-checker'),
   ...
   */
	function get_debug_info(){
		...
		$debug[ __('PHP version', 'syrus-link-checker') ] = array(
			'state' => 'ok',
			'value' => phpversion(),
		);

		//MySQL version
		$debug[ __('MySQL version', 'syrus-link-checker') ] = array(
			'state' => 'ok',
		...
		if ( function_exists('curl_version') ){
			...
			if ( version_compare( $version['version'], '7.16.0', '<=' ) ){
				$data = array(
					...
					'message' => __('You have an old version of CURL. Redirect detection may not work properly.', 'syrus-link-checker'),
				);
			} else {
				$data = array(
					'state' => 'ok',
			...
			$data = array(
				'state' => 'warning',
				'value' => __('Not installed', 'syrus-link-checker'),
			);
		}
		$debug[ __('CURL version', 'syrus-link-checker') ] = $data;

		//Snoopy presence
		if ( class_exists('Snoopy') || file_exists(ABSPATH. WPINC . '/class-snoopy.php') ){
			$data = array(
				'state' => 'ok',
				'value' => __('Installed', 'syrus-link-checker'),
			);
		} else {
			...
			if ( function_exists('curl_init') ){
				...
				$data = array(
					'state' => 'error',
					'value' => __('Not installed', 'syrus-link-checker'),
					'message' => __('You must have either CURL or Snoopy installed for the plugin to work!', 'syrus-link-checker'),
				);
			}
		...
		if ( blcUtility::is_safe_mode() ){
			$debug['Safe mode'] = array(
				'state' => 'warning',
				'value' => __('On', 'syrus-link-checker'),
				'message' => __('Redirects may be detected as broken links when safe_mode is on.', 'syrus-link-checker'),
			);
		} else {
			$debug['Safe mode'] = array(
				'state' => 'ok',
				'value' => __('Off', 'syrus-link-checker'),
			);
		}
		...
		if ( blcUtility::is_open_basedir() ){
			$debug['open_basedir'] = array(
				'state' => 'warning',
				'value' => sprintf( __('On ( %s )', 'syrus-link-checker'), ini_get('open_basedir') ),
				'message' => __('Redirects may be detected as broken links when open_basedir is on.', 'syrus-link-checker'),
			);
		} else {
			$debug['open_basedir'] = array(
				'state' => 'ok',
		...
		if ( $synch_records == 0 ){
			$data['state'] = 'warning';
			$data['message'] = __('If this value is zero even after several page reloads you have probably encountered a bug.', 'syrus-link-checker');
		}
		...
		$all_instances = intval($wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}syrus_link_checker_instances"));
	...
	function send_admin_notification($links, $email) {
		...
		$subject = sprintf(
			__("[%s] Broken links detected", 'syrus-link-checker'),
			html_entity_decode(get_option('blogname'), ENT_QUOTES)
		);

		$body = sprintf(
			_n(
				"Syrus Link Checker has detected %d new broken link on your site.",
				"Syrus Link Checker has detected %d new broken links on your site.",
				count($links),
				'syrus-link-checker'
			),
			count($links)
	...
	function build_instance_list_for_email($instances, $max_displayed_links = 5, $add_admin_link = true){
		...
		if ( count($instances) > $max_displayed_links ){
			...
			$line = __("Here's a list of the new broken links: ", 'syrus-link-checker');
		}
		...
		foreach($instances as $instance){ /* @var blcLinkInstance $instance */
			$pieces = array(
				sprintf( __('Link text : %s', 'syrus-link-checker'), $instance->ui_get_link_text('email') ),
				sprintf( __('Link URL : <a href="%s">%s</a>', 'syrus-link-checker'), htmlentities($instance->get_url()), blcUtility::truncate($instance->get_url(), 70, '') ),
				sprintf( __('Source : %s', 'syrus-link-checker'), $instance->ui_get_source('email') ),
			);
		...
		if ( $add_admin_link ) {
			$result .= __("You can see all broken links here:", 'syrus-link-checker') . "<br>";
	...
	function send_authors_notifications($links) {
		...
		foreach($authorInstances as $author_id => $instances) {
			...
			$body = sprintf(
				_n(
					"Syrus Link Checker has detected %d new broken link in your posts.",
					"Syrus Link Checker has detected %d new broken links in your posts.",
					count($instances),
					'syrus-link-checker'
				),
				count($instances)
   ...
   */
	function load_language(){
		$this->is_textdomain_loaded = load_plugin_textdomain( 'syrus-link-checker', false, basename(dirname($this->loader)) . '/languages' );
	}
	 ...
	 */
	function check_news(){
		$url = 'http://w-shadow.com/plugin-news/syrus-link-checker-news.txt';
	...
	protected static function get_default_log_directory() {
		...
		return $uploads['basedir'] . '/syrus-link-checker';
	}
