...
if (defined('BLC_ACTIVE')) {
    trigger_error(
        'Another version of Syrus Link Checker is already active. Please deactivate it before activating this one.',
        E_USER_ERROR
    );
} else {
...
if (!function_exists('get_post_stati')) {
    trigger_error(
        'This version of Syrus Link Checker requires WordPress 3.0 or later!',
        E_USER_ERROR
    );
}
...
/**
 * Get the configuration object used by Syrus Link Checker.
 *
...
if ($blc_config_manager->options['installation_complete']) {
    ...
    function blc_print_installation_errors()
    {
        ...
        $messages = array(
            '<strong>' . __('Syrus Link Checker installation failed. Try deactivating and then reactivating the plugin.', 'syrus-link-checker') . '</strong>',
        );

        if (is_multisite() && is_plugin_active_for_network(plugin_basename(BLC_PLUGIN_FILE))) {
            $messages[] = __('Please activate the plugin separately on each site. Network activation is not supported.', 'syrus-link-checker');
            $messages[] = '';
        }
