   ...
   */
	function hook_the_content($content){
        ...
        $q = "
			...
			FROM {$wpdb->prefix}syrus_link_checker_instances AS instances JOIN {$wpdb->prefix}blc_links AS links 
   ...
   */
	function update_wrapped_object(){
		if ( is_null($this->wrapped_object) ){
			return new WP_Error(
				'no_wrapped_object',
				__('Nothing to update', 'syrus-link-checker')
			);
		}
		...
		if ( is_wp_error($post_id) ) {
			...
			return new WP_Error(
				'update_failed',
				sprintf(__('Updating post %d failed', 'syrus-link-checker'), $this->container_id)
			);
		} else {
			return true;
		}
		...
		if ( EMPTY_TRASH_DAYS ){
			...
			if ( wp_delete_post($this->container_id, true) ){
				...
				return new WP_Error(
					'delete_failed',
					sprintf(
						__('Failed to delete post "%s" (%d)', 'syrus-link-checker'),
						get_the_title($this->container_id),
						$this->container_id
					)
				);
	...
	function trash_wrapped_object(){
		if ( !EMPTY_TRASH_DAYS ){
			return new WP_Error(
				'trash_disabled',
				sprintf(
					__('Can\'t move post "%s" (%d) to the trash because the trash feature is disabled', 'syrus-link-checker'),
					get_the_title($this->container_id),
					$this->container_id
				)
			);
		...
		if ( wp_trash_post($this->container_id) ){
			...
			return new WP_Error(
				'trash_failed',
				sprintf(
					__('Failed to move post "%s" (%d) to the trash', 'syrus-link-checker'),
					get_the_title($this->container_id),
					$this->container_id
				)
			);
...
class blcAnyPostContainerManager extends blcContainerManager {
	...
	function ui_bulk_delete_message($n){
		...
		if ( function_exists('wp_trash_post') && EMPTY_TRASH_DAYS ){
			...
			if ( $this->container_type == 'post' || is_null($post_type_object) ){
				$delete_msg = _n("%d post deleted.", "%d posts deleted.", $n, 'syrus-link-checker');
			} elseif ( $this->container_type == 'page' ){
				$delete_msg = _n("%d page deleted.", "%d pages deleted.", $n, 'syrus-link-checker');
			} else {
				$delete_msg = _n('%d "%s" deleted.', '%d "%s" deleted.', $n, 'syrus-link-checker');
   ...
   */
	function ui_bulk_trash_message($n){
		...
		if ( $this->container_type == 'post' || is_null($post_type_object) ){
			$delete_msg = _n("%d post moved to the Trash.", "%d posts moved to the Trash.", $n, 'syrus-link-checker');
		} elseif ( $this->container_type == 'page' ){
			$delete_msg = _n("%d page moved to the Trash.", "%d pages moved to the Trash.", $n, 'syrus-link-checker');
		} else {
			$delete_msg = _n('%d "%s" moved to the Trash.', '%d "%s" moved to the Trash.', $n, 'syrus-link-checker');
			$type_name = ($n == 1 ? $post_type_object->labels->singular_name : $post_type_object->labels->name); 
		}
		return sprintf($delete_msg, $n, $type_name);
	}
