...
class blcParser extends blcModule {
   ...
   */
	function edit($content, $new_url, $old_url, $old_raw_url){
		return new WP_Error(
			'not_implemented',
			sprintf(__("Editing is not implemented in the '%s' parser", 'syrus-link-checker'), $this->parser_type)
		);
	}
   ...
   */
	function unlink($content, $url, $raw_url){
		return new WP_Error(
			'not_implemented',
			sprintf(__("Unlinking is not implemented in the '%s' parser", 'syrus-link-checker'), $this->parser_type)
		);
	}
