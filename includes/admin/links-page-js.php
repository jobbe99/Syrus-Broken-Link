...
function reloadDetailsRow(link_id){
	...
	details_row.find('td').html('<center><?php echo esc_js(__('Loading...' , 'syrus-link-checker')); ?></center>').load(
...
jQuery(function($){
	...
	var ajaxInProgressHtml = '<?php echo esc_js(__('Wait...', 'syrus-link-checker')); ?>';
        ...
        $.post(
			...
			function (data, textStatus){
				if (data == 'OK'){
					...
					me.html('<?php echo esc_js(__('Not broken' , 'syrus-link-checker'));  ?>');
					alert(data);
				}
			}
		);
	...
	function showLinkEditor(link_id) {
		...
		var linkText = master.data('link-text'),
			...
			noneText = '<?php echo esc_js(_x('(None)', 'link text', 'syrus-link-checker')); ?>',
			multipleLinksText = '<?php echo esc_js(_x('(Multiple links)', 'link text', 'syrus-link-checker')); ?>';
	...
	function findReplacementSuggestions(url, suggestionList) {
		var searchingText     = '<?php echo esc_js(_x('Searching...', 'link suggestions', 'syrus-link-checker')) ?>';
		var noSuggestionsText = '<?php echo esc_js(_x('No suggestions available.', 'link suggestions', 'syrus-link-checker')) ?>';
		var iaSuggestionName  = '<?php echo esc_js(_x('Archived page from %s (via the Wayback Machine)', 'link suggestions', 'syrus-link-checker')); ?>';
		...
		if (response && (typeof(response['error']) != 'undefined')){
			...
			if ( response.cnt_okay > 0 ){
				var fragment = sprintf(
					'<?php echo esc_js(__('%d instances of the link were successfully modified.', 'syrus-link-checker')); ?>',
					response.cnt_okay
				);
				msg = msg + fragment + '\n';
				if ( response.cnt_error > 0 ){
					fragment = sprintf(
						'<?php echo esc_js(__("However, %d instances couldn't be edited and still point to the old URL.", 'syrus-link-checker')); ?>',
						response.cnt_error
					);
					msg = msg + fragment + "\n";
				}
			} else {
				msg = msg + '<?php echo esc_js(__('The link could not be modified.', 'syrus-link-checker')); ?>\n';
			}

			msg = msg + '\n<?php echo esc_js(__("The following error(s) occurred :", 'syrus-link-checker')); ?>\n* ';
	...
	$('.blc-update-link-button').click(function() {
		...
		if ($.trim(newUrl) == '') {
			alert('<?php echo esc_js(__('Error: Link URL must not be empty.', 'syrus-link-checker')); ?>');
			urlField.focus();
			return;
		}
    ...
    $(".blc-unlink-button").click(function () { 
    	var me = this;
    	var master = $(me).parents('.blc-row');
		$(me).html('<?php echo esc_js(__('Wait...' , 'syrus-link-checker')); ?>');
			...
			function (data, textStatus){
				...
				if ( data && (typeof(data['error']) != 'undefined') ){
					...
					if ( data.errors.length == 0 ){
						...
						if ( data.cnt_okay > 0 ){
							msg = msg + sprintf(
								'<?php echo esc_js(__("%d instances of the link were successfully unlinked.", 'syrus-link-checker')); ?>\n', 
								data.cnt_okay
							);
							
							if ( data.cnt_error > 0 ){
								msg = msg + sprintf(
									'<?php echo esc_js(__("However, %d instances couldn't be removed.", 'syrus-link-checker')); ?>\n',
									data.cnt_error
								);
							}
						} else {
							msg = msg + '<?php echo esc_js(__("The plugin failed to remove the link.", 'syrus-link-checker')); ?>\n';
						}
														
						msg = msg + '\n<?php echo esc_js(__("The following error(s) occured :", 'syrus-link-checker')); ?>\n* ';
				...
				$(me).html('<?php echo esc_js(__('Unlink' , 'syrus-link-checker')); ?>'); 
			}
		);
    });
	...
	$('#blc-create-filter').click(function(){
		var filter_name = prompt("<?php echo esc_js(__("Enter a name for the new custom filter", 'syrus-link-checker')); ?>", "");
		if ( filter_name ){
	...
	$('#blc-delete-filter').click(function(){
		...
		echo esc_js(
			html_entity_decode(
				__("You are about to delete the current filter.\n'Cancel' to stop, 'OK' to delete", 'syrus-link-checker'),
				ENT_QUOTES,
	...
	$('#bulk-edit input[type="submit"]').click(function(e){
		if( $('#bulk-edit input[name="search"]').val() == '' ){
			alert('<?php echo esc_js(__('Enter a search string first.', 'syrus-link-checker')); ?>');
		...
		if ($('tbody th.check-column input:checked').length == 0){
			alert('<?php echo esc_js(__('Select one or more links to edit.', 'syrus-link-checker')); ?>');
			e.preventDefault();
		}
	});
