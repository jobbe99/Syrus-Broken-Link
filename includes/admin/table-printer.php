...
class blcTablePrinter {
	...
	function navigation($table_compact = false, $suffix = ''){
		...
		echo '<div class="tablenav">',
				...
				' <input type="submit" name="doaction', $suffix ,'" id="doaction',$suffix,'" value="', 
					esc_attr(__('Apply', 'syrus-link-checker')),
		...
		<div class="view-switch">
			<a
				...
				title="<?php echo esc_attr(__('Compact View', 'syrus-link-checker')); ?>">
			</a>
			<a
				href="<?php echo esc_url(add_query_arg('compact', '0', $_SERVER['REQUEST_URI'])) ?>"
				class="view-excerpt <?php if ( !$table_compact ) echo 'current'; ?>"
				title="<?php echo esc_attr(__('Detailed View', 'syrus-link-checker')); ?>">
			</a>
		</div>
	...
	function setup_columns(){
		$this->columns = array(
			'status' => array(
				'heading' => __('Status', 'syrus-link-checker'),
				'content' => array($this, 'column_status'),
			),
			
			'new-url' => array(
		 		'heading' => __('URL', 'syrus-link-checker'),
			...
			'used-in' => array(
				'heading' => __('Source', 'syrus-link-checker'),
				'class' => 'column-title',
				'content' => array($this, 'column_used_in'),
			),
			
			'new-link-text' => array(
				'heading' => __('Link Text', 'syrus-link-checker'),
			...
			'redirect-url' => array(
				'heading' => __('Redirect URL', 'syrus-link-checker'),
	...
	function prepare_nav_html(){
		...
		$bulk_actions = array(
			'-1' => __('Bulk Actions', 'syrus-link-checker'),
			"bulk-edit" => __('Edit URL', 'syrus-link-checker'),
			"bulk-recheck" => __('Recheck', 'syrus-link-checker'),
			"bulk-deredirect" => __('Fix redirects', 'syrus-link-checker'),
			"bulk-not-broken" => __('Mark as not broken', 'syrus-link-checker'),
			"bulk-dismiss" => __('Dismiss', 'syrus-link-checker'),
			"bulk-unlink" => __('Unlink', 'syrus-link-checker'),
		);
		if ( EMPTY_TRASH_DAYS ){
			$bulk_actions["bulk-trash-sources"] = __('Move sources to Trash', 'syrus-link-checker');
		} else {
			$bulk_actions["bulk-delete-sources"] = __('Delete sources', 'syrus-link-checker');
		}
		...
		if ( $page_links ) {
			...
			$this->pagination_html .= sprintf( 
				'<span class="displaying-num">' . __( 'Displaying %s&#8211;%s of <span class="current-link-count">%s</span>', 'syrus-link-checker' ) . '</span>%s',
	...
	function bulk_edit_form($visible_columns){
		...
		<fieldset>
			<h4><?php _e('Bulk Edit URLs', 'syrus-link-checker'); ?></h4>
			<label>
				<span class="title"><?php _e('Find', 'syrus-link-checker'); ?></span>
				<input type="text" name="search" class="text">
			</label>
			<label>
				<span class="title"><?php _e('Replace with', 'syrus-link-checker'); ?></span>
			...
			<div id="bulk-edit-options">
				...
				<label>
					...
					<?php _e('Case sensitive', 'syrus-link-checker'); ?>
				</label>
				<label>
					<input type="checkbox" name="regex">
					<?php _e('Regular expression', 'syrus-link-checker'); ?>
				</label>
			</div>
		</fieldset>			
		
		<p class="submit inline-edit-save">
			<a href="#bulk-edit" class="button-secondary cancel alignleft" title="<?php echo esc_attr(__('Cancel', 'syrus-link-checker')); ?>" accesskey="c"><?php _e('Cancel', 'syrus-link-checker'); ?></a>
			<input type="submit" name="bulk_edit" class="button-primary alignright" value="<?php 
				_e('Update', 'syrus-link-checker'); 
			?>" accesskey="s">
		</p>
		</div>
		</td></tr>
	...
	public static function details_row_contents($link){
		...
		<div class="blc-detail-container">
			<div class="blc-detail-block" style="float: left; width: 49%;">
		    	...
		    	<li><strong><?php _e('HTTP code', 'syrus-link-checker'); ?>:</strong>
		    	<span class='http_code'><?php 
		    		print $link->http_code; 
		    	?></span></li>
		    	
		    	<li><strong><?php _e('Response time', 'syrus-link-checker'); ?>:</strong>
		    	<span class='request_duration'><?php 
		    		printf( __('%2.3f seconds', 'syrus-link-checker'), $link->request_duration); 
		    	?></span></li>
		    	
		    	<li><strong><?php _e('Final URL', 'syrus-link-checker'); ?>:</strong>
		    	<span class='final_url'><?php 
		    		print $link->final_url; 
		    	?></span></li>
		    	
		    	<li><strong><?php _e('Redirect count', 'syrus-link-checker'); ?>:</strong>
		    	<span class='redirect_count'><?php 
		    		print $link->redirect_count; 
		    	?></span></li>
		    	
		    	<li><strong><?php _e('Instance count', 'syrus-link-checker'); ?>:</strong>
				...
				<?php 
					printf(
						_n('This link has failed %d time.', 'This link has failed %d times.', $link->check_count, 'syrus-link-checker'),
						$link->check_count
					);
					...
					printf(
						__('This link has been broken for %s.', 'syrus-link-checker'),
			...
			<div class="blc-detail-block" style="float: right; width: 50%;">
		    	<ol style='list-style-type: none;'>
		    		<li><strong><?php _e('Log', 'syrus-link-checker'); ?>:</strong>
	...
	function column_status($link, $instances){
		printf(
			...
			esc_attr(__('Show more info about this link', 'syrus-link-checker'))
		);
		...
		if ( $link->last_check != 0 ){
			$last_check = _x('Checked', 'checked how long ago', 'syrus-link-checker') . ' ';
		...
		if ( $link->broken ){
			...
			printf(
				...
				__('Broken for', 'syrus-link-checker'),
				$broken_for
			);
		}
		...
		printf(
			...
			esc_attr(__('Show more info about this link', 'syrus-link-checker')),
			_x('Details', 'link in the "Status" column', 'syrus-link-checker')
		);
		echo '</div>';
	}
	...
	function column_new_url($link){
    	...
    	//Output inline action links for the link/URL                  	
      	...
      	$actions['edit'] = "<a href='javascript:void(0)' class='blc-edit-button' title='" . esc_attr( __('Edit this link' , 'syrus-link-checker') ) . "'>". __('Edit URL' , 'syrus-link-checker') ."</a>";
      	
      	$actions['delete'] = "<a class='submitdelete blc-unlink-button' title='" . esc_attr( __('Remove this link from all posts', 'syrus-link-checker') ). "' ".
			"href='javascript:void(0);'>" . __('Unlink', 'syrus-link-checker') . "</a>";

		if ( $link->broken || $link->warning ){
			$actions['blc-discard-action'] = sprintf(
				'<a href="#" title="%s" class="blc-discard-button">%s</a>',
				esc_attr(__('Remove this link from the list of broken links and mark it as valid', 'syrus-link-checker')),
				__('Not broken', 'syrus-link-checker')
			);
		}
		...
		if ( !$link->dismissed && ($link->broken || $link->warning || ($link->redirect_count > 0)) ) {
			$actions['blc-dismiss-action'] = sprintf(
				...
				esc_attr(__('Hide this link and do not report it again unless its status changes' , 'syrus-link-checker')),
				__('Dismiss', 'syrus-link-checker')
			);
		} else if ( $link->dismissed ) {
			$actions['blc-undismiss-action'] = sprintf(
				'<a href="#" title="%s" class="blc-undismiss-button">%s</a>',
				esc_attr(__('Undismiss this link', 'syrus-link-checker')),
				__('Undismiss', 'syrus-link-checker')
			);
		}

		$actions['blc-recheck-action'] = sprintf(
			'<a href="#" class="blc-recheck-button">%s</a>',
			__('Recheck', 'syrus-link-checker')
		);
		...
		if ( $link->redirect_count > 0 && !empty($link->final_url) && ($link->url != $link->final_url) ) {
			...
			$actions['blc-deredirect-action'] = sprintf(
				...
				__('Replace this redirect with a direct link', 'syrus-link-checker'),
				_x('Fix redirect', 'link action; replace one redirect with a direct link', 'syrus-link-checker')
			);
		}
		...
		<div class="blc-url-editor-buttons">
			<input type="button" class="button-secondary cancel alignleft blc-cancel-button" value="<?php echo esc_attr(__('Cancel', 'syrus-link-checker')); ?>" />
			<input type="button" class="button-primary save alignright blc-update-url-button" value="<?php echo esc_attr(__('Update URL', 'syrus-link-checker')); ?>" />
	...
	function column_used_in($link, $instances){
		...
		if ( !empty($instances) ){
			...
			_e("[An orphaned link! This is a bug.]", 'syrus-link-checker');
		}
	}
	...
	protected function inline_editor($visible_columns) {
			...
			<tr id="blc-inline-edit-row" class="blc-inline-editor">
				<td class="blc-colspan-change" colspan="<?php echo count($visible_columns); ?>">
					<div class="blc-inline-editor-content">
						<h4><?php echo _x('Edit Link', 'inline editor title', 'syrus-link-checker'); ?></h4>

						<label>
							<span class="title"><?php echo _x('Text', 'inline link editor', 'syrus-link-checker'); ?></span>
							<span class="blc-input-text-wrap"><input type="text" name="link_text" value="" class="blc-link-text-field" /></span>
						</label>

						<label>
							<span class="title"><?php echo _x('URL', 'inline link editor', 'syrus-link-checker'); ?></span>
						...
						<div class="blc-url-replacement-suggestions" style="display: none;">
							<h4><?php echo _x('Suggestions', 'inline link editor', 'syrus-link-checker'); ?></h4>
						...
						<div class="submit blc-inline-editor-buttons">
							<input type="button" class="button-secondary cancel alignleft blc-cancel-button" value="<?php echo esc_attr(__('Cancel', 'syrus-link-checker')); ?>" />
							<input type="button" class="button-primary save alignright blc-update-link-button" value="<?php echo esc_attr(__('Update', 'syrus-link-checker')); ?>" />
		...
		<ul id="blc-suggestion-template" style="display: none;">
			<li>
				<input type="button" class="button-secondary blc-use-url-button" value="<?php echo esc_attr(__('Use this URL', 'syrus-link-checker')); ?>" />
