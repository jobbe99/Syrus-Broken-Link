...
jQuery(function($){
		...
		$.getJSON(
			...
			function (data, textStatus){
				if ( data && ( typeof(data['text']) != 'undefined' ) ){
					...
					$('#wsblc_full_status').html('<?php _e('[ Network error ]', 'syrus-link-checker'); ?>');
				}
	...
	toggleButton.click(function(){
		...
		box.toggle();
		if( box.is(':visible') ){
			toggleButton.text('<?php _e('Hide debug info', 'syrus-link-checker'); ?>');
		} else {
			toggleButton.text('<?php _e('Show debug info', 'syrus-link-checker'); ?>');
		}
