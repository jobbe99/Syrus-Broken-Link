   ...
   */
	...
	static function drop_tables(){
		...
		$tables = array(
			...
			$wpdb->prefix . 'syrus_link_checker_instances',
		...
		if ( $rez === false ){
			$error = sprintf(
				__("Failed to delete old DB tables. Database error : %s", 'syrus-link-checker'),
				$wpdb->last_error
			); 
	...
	static function upgrade_095($trigger_errors = false){
		...
		$q = "
			UPDATE 
				{$wpdb->prefix}syrus_link_checker_instances AS instances 
