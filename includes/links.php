...
//				$blclog->debug(__CLASS__ .':' . __FUNCTION__ . ' Link not found.');
	...
	function check( $save_results = true ){
        ...
        */
        if ( $this->being_checked ) {
        	...
        	$error_notice = "[" . __("The plugin script was terminated while trying to check the link.", 'syrus-link-checker') . "]";
		...
		if ( is_null($checker) ){
			...
			$this->broken = false;
			$this->being_checked = false;
			$this->log = __("The plugin doesn't know how to check this type of link.", 'syrus-link-checker');
   ...
   */
	private function status_changed($broken, $new_result_hash = ''){
		...
		if ( $this->broken || $this->warning ) {
			$this->log .= "\n" . __("Link is broken.", 'syrus-link-checker');
		} else {
			$this->log .= "\n" . __("Link is valid.", 'syrus-link-checker');
		}
	}
   ...
   */
	function edit($new_url, $new_text = null){
		if ( !$this->valid() ){
			return new WP_Error(
				'link_invalid',
				__("Link is not valid", 'syrus-link-checker')
			);
		}
		...
		if (empty($instances)) {
			return array(
				...
				'errors' => array(
					new WP_Error(
						'no_instances_found',
						__('This link can not be edited because it is not used anywhere on this site.', 'syrus-link-checker')
					)
				)
			);
		};
			...
			return array(
				...
				'errors' => array(
					new WP_Error(
						...
						__('Failed to create a DB entry for the new URL.', 'syrus-link-checker')
					)
				)
			);
		}
   ...
   */ 
	function deredirect(){
		...
		if ( ($this->redirect_count <= 0) || empty($this->final_url) ){
			return new WP_Error(
				'not_redirect',
				__("This link is not a redirect", 'syrus-link-checker')
			);
		}
			...
			if ( $rez ){
				...
				return array(
					...
					'errors' => array(
						new WP_Error(
							"deletion_failed",
							__("Couldn't delete the link's database record", 'syrus-link-checker')
						)
					), 
				);
			}
	...
	function forget($remove_instances = true){
		...
		if ( !empty($this->link_id) ){
			...
			if ( $remove_instances ){
				...
				$wpdb->query( $wpdb->prepare("DELETE FROM {$wpdb->prefix}syrus_link_checker_instances WHERE link_id=%d", $this->link_id) );
			}
   ...
   */
	...
	function analyse_status(){
		...
		$text = _x('Unknown', 'link status', 'syrus-link-checker');
		...
		if ( isset($this->status_text) && !empty($this->status_text) && !empty($this->status_code) ){
			...
			if ( $this->broken || $this->warning ){
				...
				$text = __('Unknown Error', 'link status', 'syrus-link-checker');
				
				if ( $this->timeout ){
					
					$text = __('Timeout', 'syrus-link-checker');
					$code = BLC_LINK_STATUS_WARNING;
				...
				if ( !$this->last_check ) {
					$text = __('Not checked', 'syrus-link-checker');
					$code = BLC_LINK_STATUS_UNKNOWN;
				} elseif ( $this->false_positive ) {
					$text = __('False positive', 'syrus-link-checker');
					$code = BLC_LINK_STATUS_UNKNOWN;
				} else {
					$text = _x('OK', 'link status', 'syrus-link-checker');
...
function blc_cleanup_links( $link_id = null ){
	...
	$q = "DELETE FROM {$wpdb->prefix}blc_links
			USING {$wpdb->prefix}blc_links LEFT JOIN {$wpdb->prefix}syrus_link_checker_instances 
				ON {$wpdb->prefix}syrus_link_checker_instances.link_id = {$wpdb->prefix}blc_links.link_id
			WHERE
				{$wpdb->prefix}syrus_link_checker_instances.link_id IS NULL";
