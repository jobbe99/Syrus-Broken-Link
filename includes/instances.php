   ...
   */
	function __construct($arg = null){
		...
		if (is_int($arg)){
			...
			$q = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}syrus_link_checker_instances WHERE instance_id=%d LIMIT 1", $arg);
   ...
   */
	...
	function edit($new_url, $old_url = '', $new_text = null){
		...
		if ( is_null($container) ){
			return new WP_Error(
				'container_not_found',
				sprintf(__("Container %s[%d] not found", 'syrus-link-checker'), $this->container_type, $this->container_id)
			);
		}
		...
		if ( is_null($parser) ){
			return new WP_Error(
				'parser_not_found', 
				sprintf(__("Parser '%s' not found.", 'syrus-link-checker'), $this->parser_type)
			);
		}
   ...
   */
	function forget(){
		...
		if ( !empty($this->instance_id) ) {
			$rez = $wpdb->query( $wpdb->prepare("DELETE FROM {$wpdb->prefix}syrus_link_checker_instances WHERE instance_id=%d", $this->instance_id) );
			return $rez;
		} else {
			return false;
		}
   ...
   */
	function save(){
		...
		if ( $this->is_new ){
			...
			$q = "
			INSERT INTO {$wpdb->prefix}syrus_link_checker_instances
			...
			$q = "UPDATE {$wpdb->prefix}syrus_link_checker_instances
...
function blc_get_instances( $link_ids, $purpose = '', $load_containers = false, $load_wrapped_objects = false, $include_invalid = false ){
	...
	$q = "SELECT * FROM {$wpdb->prefix}syrus_link_checker_instances WHERE link_id IN ($link_ids_in)";
...
function blc_get_usable_instance_count(){
	...
	$q = "SELECT COUNT(instance_id) FROM {$wpdb->prefix}syrus_link_checker_instances WHERE 1";
...
function blc_cleanup_instances(){
	...
	$q = "DELETE instances.*
		  FROM 
  			{$wpdb->prefix}syrus_link_checker_instances AS instances LEFT JOIN {$wpdb->prefix}blc_synch AS synch
	...
	$q = "DELETE instances.*
	      FROM {$wpdb->prefix}syrus_link_checker_instances AS instances
	      WHERE
